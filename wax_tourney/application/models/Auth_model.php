...
class Auth_model extends CI_Model
{
	...
	public function new_account($credentials){
		...
		if( ! $messages["errors"] ){
			$sql = "SELECT * FROM `wax_users` WHERE username = '".$username."'";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
				$messages["errors"] = 1;
			...
			$sql = "SELECT * FROM `wax_users` WHERE email = '".$email."'";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
				$messages["errors"] = 1;
		...
		if( ! $messages["errors"] ){
			$sql = "INSERT INTO `wax_users` (`id`, `username`, `email`, `password`, `role`) 
